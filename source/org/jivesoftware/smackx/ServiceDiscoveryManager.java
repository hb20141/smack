...
import org.jivesoftware.custom.iquery.ConversationOperation;
import org.jivesoftware.custom.packet.MUCOperationResult;
import org.jivesoftware.custom.iquery.RetrieveConversations;
import org.jivesoftware.custom.iquery.RetrieveGroupConversations;
import org.jivesoftware.custom.provider.ConversationOperationProvider;
import org.jivesoftware.custom.packet.HBSession;
import org.jivesoftware.custom.iquery.MUCOperation;
import org.jivesoftware.custom.iquery.RetrieveChatHistory;
import org.jivesoftware.custom.iquery.RetrieveHBSession;
import org.jivesoftware.custom.iquery.RetrieveConversationDetail;
import org.jivesoftware.custom.iquery.RetrieveConversationMembers;
...
public class ServiceDiscoveryManager {
    ...
    public ChatHistory retrieveChatHistory(String targetJID, String UTC, String limit, String index, String order) throws XMPPException {
        Connection connection = ServiceDiscoveryManager.this.connection.get();
        if (connection == null) throw new XMPPException("Connection instance already gc'ed");

        RetrieveChatHistory retrieve = new RetrieveChatHistory(targetJID, UTC, limit, index, order);
    ...
    public Conversations retrieveConversations(String max, String index, String before) throws XMPPException {
        Connection connection = ServiceDiscoveryManager.this.connection.get();
        if (connection == null) throw new XMPPException("Connection instance already gc'ed");

        RetrieveConversations retrieve = new RetrieveConversations(max, index, before);
        retrieve.setType(IQ.Type.GET);
    ...
    public ConversationOperationResult readConversation(String target) throws XMPPException {
        Connection connection = ServiceDiscoveryManager.this.connection.get();
        if (connection == null) throw new XMPPException("Connection instance already gc'ed");

        ConversationOperation retrieve = new ConversationOperation(target,
                ConversationOperationProvider.COP_READ_ELEMENT, "");
        // Create a packet collector to listen for a response.
        PacketCollector collector =
                connection.createPacketCollector(new PacketIDFilter(retrieve.getPacketID()));
        ...
        return (ConversationOperationResult)result;
    }

    public ConversationOperationResult removeConversation(String target) throws XMPPException {
        Connection connection = ServiceDiscoveryManager.this.connection.get();
        if (connection == null) throw new XMPPException("Connection instance already gc'ed");

        ConversationOperation retrieve = new ConversationOperation(target,
                ConversationOperationProvider.COP_REMOVE_ELEMENT, "");
        // Create a packet collector to listen for a response.
        PacketCollector collector =
                connection.createPacketCollector(new PacketIDFilter(retrieve.getPacketID()));
    ...
    public ConversationOperationResult renameConversation(String target, String name) throws XMPPException {
        Connection connection = ServiceDiscoveryManager.this.connection.get();
        if (connection == null) throw new XMPPException("Connection instance already gc'ed");

        ConversationOperation retrieve = new ConversationOperation(target,
                ConversationOperationProvider.COP_RENAME_ELEMENT, name);
        // Create a packet collector to listen for a response.
        PacketCollector collector =
                connection.createPacketCollector(new PacketIDFilter(retrieve.getPacketID()));
    ...
    public ConversationOperationResult notifyConversation(String target, boolean notify) throws XMPPException {
        Connection connection = ServiceDiscoveryManager.this.connection.get();
        if (connection == null) throw new XMPPException("Connection instance already gc'ed");

        ConversationOperation retrieve = new ConversationOperation(target,
                ConversationOperationProvider.COP_NOTIFY_ELEMENT, String.valueOf(notify));
        // Create a packet collector to listen for a response.
        PacketCollector collector =
                connection.createPacketCollector(new PacketIDFilter(retrieve.getPacketID()));
    ...
    public ConversationDetail retrieveConversationDetail(String target) throws XMPPException {
        Connection connection = ServiceDiscoveryManager.this.connection.get();
        if (connection == null) throw new XMPPException("Connection instance already gc'ed");

        RetrieveConversationDetail retrieve = new RetrieveConversationDetail(target);
    ...
    public ConversationMembers retrieveConversationMembers(String target) throws XMPPException {
        Connection connection = ServiceDiscoveryManager.this.connection.get();
        if (connection == null) throw new XMPPException("Connection instance already gc'ed");

        RetrieveConversationMembers retrieve = new RetrieveConversationMembers(target);
        ...
        return (ConversationMembers)result;
    }

    public Conversations retrieveGroupConversationList(String max, String index, String before) throws XMPPException {
        Connection connection = ServiceDiscoveryManager.this.connection.get();
        if (connection == null) throw new XMPPException("Connection instance already gc'ed");

        RetrieveGroupConversations retrieve = new RetrieveGroupConversations(max, index, before);
        retrieve.setType(IQ.Type.GET);
    ...
    public HBSession retriveHBSession(String with, String name)  throws  XMPPException {
        Connection connection = ServiceDiscoveryManager.this.connection.get();
        if (connection == null) throw new XMPPException("Connection instance already gc'ed");

        RetrieveHBSession retrieve = new RetrieveHBSession(with, name);
    ...
    public MUCOperationResult retrieveMUCOperation(String sid, List<String> userIds, String operation) throws XMPPException {
        Connection connection = ServiceDiscoveryManager.this.connection.get();
        if (connection == null) throw new XMPPException("Connection instance already gc'ed");

        MUCOperation retrieve = new MUCOperation(sid, userIds, operation);
        retrieve.setType(IQ.Type.SET);
        ...
        return (MUCOperationResult)result;
    }
